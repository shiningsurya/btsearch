...
cdef class FDMT_negative:
    ...
    def __init__ (self, float fch1, float foff, unsigned int nchans):
        ...
        self.fmax  = fch1
        if foff > 0:
            raise ValueError ("FDMT_negative requires negative foff")
        else:
            foff   = -foff
...
cdef class FDMT_positive:
    ...
    def __init__ (self, float fch1, float foff, unsigned int nchans):
        ...
        self.fmin  = fch1
        if foff < 0:
            raise ValueError ("FDMT_positive requires foff positive")
        self.fmax  = fch1 + (nchans*foff)
        self.fres  = foff
        self.F     = nchans
        self.f     = np.log2 (self.F)
    ...
    def Bowtie_delays (self, np.ndarray[cf_t, ndim=2] Image, np.ndarray[np.uint32_t, ndim=1] delays):
        ...
        cdef Py_ssize_t nsamps             = Image.shape[1]
        cdef Py_ssize_t dsize              = delays.shape[0]
        cdef Py_ssize_t maxdt              = delays[0] + 1
        cdef np.ndarray fbt                = self.__worker__(Image, maxdt)
        # dmrange selection
        cdef Py_ssize_t ddnsamps           = nsamps - maxdt
        cdef np.ndarray[cf_t, ndim=2] bt   = np.zeros ((delays.size, ddnsamps), dtype=f_t)
